...
module.exports = {
    async listarResumoAdmin(request, response) {
        try {
            const sqlUsuarios = `SELECT COUNT(*) AS totalUsuarios FROM USUARIOS WHERE usu_status = 1`;
            const sqlEmpresas = `SELECT COUNT(*) AS totalEmpresas FROM EMPRESAS WHERE emp_status = 1`;
            const sqlDocumentos = `SELECT COUNT(*) AS totalDocumentos FROM DOCUMENTOS WHERE doc_status = 1`;
            
            const sqlFaturamento = `
                SELECT COALESCE(SUM(fin_valor_total), 0) AS totalFaturamento
                FROM FINANCEIRO WHERE fin_categoria = 'Faturamento' AND fin_status = 1
            `;
            const sqlImpostos = `
                SELECT COALESCE(SUM(fin_valor_total), 0) AS totalImpostos
                FROM FINANCEIRO WHERE fin_categoria = 'Imposto' AND fin_status = 1
            `;
            const sqlDespesas = `
                SELECT COALESCE(SUM(fin_valor_total), 0) AS totalDespesas
                FROM FINANCEIRO WHERE fin_categoria = 'Despesa' AND fin_status = 1
            `;
            ...
            const dados = {
                ...
                totalFaturamento: Number(resFaturamento[0]?.totalFaturamento || 0),
                totalImpostos: Number(resImpostos[0]?.totalImpostos || 0),
                totalDespesas: Number(resDespesas[0]?.totalDespesas || 0)
            };
        ...
        try {
            const sql = `
                SELECT
                    ...
                    CASE WHEN e.emp_tipo = 1 THEN 6750.00 ELSE 20000.00 END AS limiteMensal,
                    CASE
                        WHEN COALESCE(SUM(f.fin_valor_total), 0) >= CASE WHEN e.emp_tipo = 1 THEN 6750.00 ELSE 20000.00 END * 0.8 THEN 'Risco'
                        WHEN COALESCE(SUM(f.fin_valor_total), 0) >= CASE WHEN e.emp_tipo = 1 THEN 6750.00 ELSE 20000.00 END * 0.5 THEN 'Atenção'
                        ELSE 'Saudável'
                    END AS situacao
                 FROM EMPRESAS e
                 LEFT JOIN DOCUMENTOS d ON d.emp_id = e.emp_id AND d.doc_status = 1
                 LEFT JOIN FINANCEIRO f ON f.doc_id = d.doc_id
    ...
    async listarUltimosDocumentos(request, response) {
        try {
            // Garante que o limite é um número limpo e seguro contra SQL Injection/Erros de Driver
            ...
            const sql = `
                ...
                ORDER BY d.doc_data_upload DESC, d.doc_id DESC
                LIMIT ${limit}
            `;
        ...
        try {
            const sql = `
                SELECT
                    ...
                    p.praz_status
                FROM PRAZOS p
                INNER JOIN EMPRESAS e ON e.emp_id = p.emp_id
    ...
    async listarAuditoriaRecente(request, response) {
        try {
            // SOLUÇÃO DO BUG: Força o limit a virar um número inteiro limpo injetado de forma segura
            const limit = Math.min(Math.max(parseInt(request.query.limit) || 10, 1), 100);
    ...
    async listarFinanceiroMensal(request, response) {
        try {
            ...
            const sql = `
                ...
                AND MONTH(fin_data_emissao) = ? AND fin_status = 1
            ...
            rows.forEach(item => {
                if (item.fin_categoria === 'Faturamento') dados.faturamento = Number(item.total);
                if (item.fin_categoria === 'Imposto') dados.impostos = Number(item.total);
                if (item.fin_categoria === 'Despesa') dados.despesas = Number(item.total);
            });

            dados.saldo = dados.faturamento - dados.impostos - dados.despesas;
